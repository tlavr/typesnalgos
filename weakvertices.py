...
class SimpleGraph:
    ...
    def WeakVertices(self, vtx = None, isFirst = True):
        # возвращает список узлов вне треугольников
        if isFirst:
            for ii in range(self.max_vertex):
                if self.vertex[ii] is not None:
                    self.vertex[ii].Hit = False
                    if isFirst:
                        vtx = ii
                        isFirst = False
            self.nonTr = []
        if vtx is None:
            return []
        self.vertex[vtx].Hit = True
        vtxAdjs = []
        isTriangle = False
        for ii in range(self.max_vertex):
            if self.vertex[ii] is not None:
                if self.IsEdge(vtx, ii):
                    vtxAdjs.append(ii)
        for ii in range(vtxAdjs.__len__()):
            for jj in range(ii+1, vtxAdjs.__len__()):
                if self.IsEdge(vtxAdjs[ii], vtxAdjs[jj]):
                    isTriangle = True
                    self.vertex[vtxAdjs[ii]].Hit = True
                    self.vertex[vtxAdjs[jj]].Hit = True
        if not isTriangle:
            self.nonTr.append(self.vertex[vtx])
        for ii in range(self.max_vertex):
            if self.vertex[ii] is not None:
                if self.vertex[ii].Hit is False:
                    self.WeakVertices(ii, False)
        return self.nonTr
